...
class BitcoinAdapter implements NodeAdapterInterface
{
    ...
    public function fixedUpdate($data)
    {
        ...
        foreach ($accounts as $account) {
            ...
            foreach ($transactions as $transaction) {
                ...
                $backendTransactions[$transaction['address']]['transactions'][] = [
    ...
    public function update($data)
    {
        ...
        foreach ($transactionIds as $transactionId) {
            ...
            if ($account) {
                $this->db->addOrUpdateTransaction($rawTransaction['blockhash'], $rawTransaction['txid'], $rawTransaction['locktime'], $rawTransaction['confirmations'] ?? 0, '', $to, $amount, '');
                $balance = $this->node->getBalance($account->getName());
                $account->setLastBalance(Currency::showMinorCurrency($this->currency, $balance));
